function jsp_selecttab(pdiv){
    var sclass before=pdiv.class Name.substring(0,pdiv.class Name.indexOf('_')); 
    var itab=0;
    var btab=false;
    var i=0;
    ...
    for(var i=0;i<pset.length;i++){
        if(pset[i].id==pdiv.id){
            if(!btab){itab++;}
            if(pset[i]==pdiv){
                pset[i].class Name=sclass before+'_sel';
                btab=true;
            }
            else{
                pset[i].class Name=sclass before+'_nor';
            }
        }
    }
    var itemp=0;
    ...
    for(var i=0;i<pset.length;i++){
        if(pset[i].id==(pdiv.id+'_content')){
            itemp++;
            if(itemp==itab){
                pset[i].class Name=sclass before+'_content_sel';
            }
            else{
                pset[i].class Name=sclass before+'_content_nor';
            }
        }
    }
};
